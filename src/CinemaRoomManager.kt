...
fun main(args: Array<String>) {
    ...
    val numberOfSeatInEachRow = readln().toInt()
    val totalNumberOfSeat = numberOfRows * numberOfSeatInEachRow

    // var profit: Int

    totalIncome = if (totalNumberOfSeat <= 60) {
        // Screen Room
        totalNumberOfSeat * 10
    } else {
        // Larger Room
        if(numberOfRows % 2 == 0) {
            ((numberOfRows / 2) * numberOfSeatInEachRow * 10) + ((totalNumberOfSeat / 2) * 8)
        } else {
            ((numberOfRows / 2) * numberOfSeatInEachRow * 10) + (((numberOfRows / 2) + 1) * numberOfSeatInEachRow * 8)
        }
    }
    ...
    do{
        // Print Menu
        printMenu()
        var selectedMenuItem = readln().toInt()
...
fun buyTicket(totalNumberOfSeat: Int, numberOfRows: Int) {
    ...
    val seatNumberInRow = readln().toInt()
...
fun calculateTicketPrice(totalNumberOfSeat: Int, rowNumber: Int, numberOfRows: Int) {
    ticketPrice = if (totalNumberOfSeat <= 60) {
        // Screen Room
        10
    } else {
        // Larger Room
